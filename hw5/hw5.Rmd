...
output: 
  html_document
---
19/20. Your work is very well explained.
...
I'm not sure how we're supposed to show our work. But I did the magnitude of vectors equation we've been using in multi-variable calculus and found 6.40. Assuming objects were the rows.

```{r}
dist(X) #The answer is correct
```

b) Find the Manhattan distance between the first and third objects. Round the answer to have two decimal points.

Manhattan distance is done very similarly but it's not the same. Assuming objects are rows. I found the distance between the first and third objects via a manhattan measurement to be 10.

```{r}
dist(X, method = "manhattan") #This answer is also correct
```

c) Find the Minkowski distance ($\rho$ = 3) between the second and third objects. Round the answer to have two decimal points.

Assuming that 3 = p here, cause I'm not sure what else that thing in parentheses is supposed to mean otherwise. The answer is cube root of 11 or 2.22
#The thing in parentheses is the greek letter rho which looks like a lower-case letter "p". 

```{r}
dist(X, method = "minkowski", p = 3) #The answer is correct
```
...
#There are only 9 objects within Vans93
```
...
It'll be 1 (the distance between 1 and 2).

b) Farthest-neighbor

It's 1. The distance between 1 and 3.
Furthest neighbor distance is 9-between 1 and 3. (-1)

c) Average-neighbor

It'll be 4.1 cause I summed up all of the distances between each cluster and divided them by 6 (the amount of possible differences) and that's what I got.
...
I simply found where there were five branches on the dendrogram and split the clusters likewise. Clusters 3 through 5 are all only one word, were I doing this any other way I probably would put them into their own entropy group because of that individuality split.
...
I kinda prefer the euclidean method if only because I don't have to write as much in order for the program to work.
