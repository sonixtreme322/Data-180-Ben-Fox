...
```{r}
#These might be causing issues, because when I did these exact commands in class it did weird things to the data that the Professor did not have to deal with. 
...
#It's a matrix that returns the amount of terms within a given corpus. Also returns other notable variables such as the corpus's sparsity. 
```
...
install.packages("corpus") #I literally can't continue, I've run out of time, and I need to update R itself to get this specific package at all. I'm sorry, I was hoping to do well but everything is happening all the time for the past week and I haven't had time to put the time into this homework that I didn't realize it'd need. I'm so so sorry.
library("quanteda")
library('corpus')

### Its okay. You have done well on the portions of the HW you attempted. Good job. 
```
